...
{
    connect(serialPort_command, &QSerialPort::readyRead, this, &recSerial::serial_Read_Data);//当有数据来时，触发接收槽函数；
}
                    ...
                    case 4:
                        ...
                        if(output_cnt >= pkg_length+1){
                            if(crc_sum == pRxByte ){
                                ...
                                else if(0x35 == output_array[0])
                                {
                                     upgraderesp(output_array);
                                }
                                else if(output_array[0]==0x33)
                                {
                                    exportfile(output_array);
                                }
                                else
                                {
